...
if (! isset($_REQUEST ['3ds'])) {
    ...
    if (empty($_SESSION ['Zahlungsart']->cModulId)) {
        $payCode = str_replace(array(
                'za_hp',
                '_jtl'
        ), '', $bestellung->Zahlungsart->cModulId);
    } else {
        $payCode = str_replace(array(
                'za_hp',
                '_jtl'
    ...
    if ((($payCode == 'cc' && HPCC_MODULE_MODE == 'DIRECT') || ($payCode == 'dc' && HPDC_MODULE_MODE == 'DIRECT') || ($payCode == 'dd' && HPDD_MODULE_MODE == 'DIRECT')) && ($payMethod == 'DB' || $payMethod == 'PA')) {
