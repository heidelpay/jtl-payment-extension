...
* @category JTL
*/
include_once('../includes/plugins/heidelpay_standard/version/110/paymentmethod/class.heidelpay_sbase.php');
    ...
    public function handleNotification($order, $paymentHash, $args)
    {
        ...
        if ($this->verifyNotification($order, $paymentHash, $args)) {
            ...
            if ((strtoupper($payCode[0]) == 'DD') && (!isset($args['TRANSACTION_SOURCE']))) {
                $language = $_SESSION['cISOSprache']=='ger'?'DE':'EN';
                if ($language == 'DE') {
                    include_once('../includes/plugins/heidelpay_standard/version/110/paymentmethod/template/heidelpay_ddMail_de.tpl');
                } else {
                    include_once('../includes/plugins/heidelpay_standard/version/110/paymentmethod/template/heidelpay_ddMail_en.tpl');
                }
                $repl = array(
