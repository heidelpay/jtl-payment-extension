...
class heidelpay_sbase extends ServerPaymentMethod
{
  ...
  public function getBillsafeBasket($order)/*{{{*/
  {
      ...
      if ($items) {
          ...
          //} else {
          $parameters[$prefix.'.AMOUNT_UNIT_GROSS'] = (int)(($item->fPreisEinzelNetto + (($item->fPreisEinzelNetto/100)*$item->fMwSt)) * 100);
                  $parameters[$prefix.'.AMOUNT_GROSS']          = (int)(($item->fPreisEinzelNetto + (($item->fPreisEinzelNetto/100)*$item->fMwSt)) * 100) * $item->nAnzahl;
          //}
