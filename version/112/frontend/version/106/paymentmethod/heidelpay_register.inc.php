...
if (! isset($_REQUEST ['hp_go'])) {
    ...
    if ($Einstellungen ['zahlungsarten'] ['zahlungsart_' . $prefixLOW . '_modulemode'] == 'DIRECT') {
        ...
        $hpActionURL = str_replace('payment.prc', '', $hp->url) . 'submitPayment.prc;jsessionid=' . $hpSID . '?FRONTEND.RESPONSE_URL=' . $data ['FRONTEND.RESPONSE_URL'] . '&IDENTIFICATION.TRANSACTIONID=' . $data ['IDENTIFICATION.TRANSACTIONID'] . '&FRONTEND.POPUP=' . $data ['FRONTEND.POPUP'] . '&FRONTEND.NEXT_TARGET=' . $data ['FRONTEND.NEXT_TARGET'] . '&frontendCancelled=false';
