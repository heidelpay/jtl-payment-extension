<?php
// include_once(PFAD_ROOT.PFAD_PLUGIN.'heidelpay_standard/class.heidelpay_pbase.php');
include_once('../includes/plugins/heidelpay_standard/version/106/paymentmethod/class.heidelpay_sbase.php');
        ...
        if (strstr($args ['PROCESSING_RESULT'], 'ACK')) {
            ...
            if ($this->verifyNotification($order, $paymentHash, $args)) {
                ...
                if ((strtoupper($payCode [0]) == 'DD') && (! isset($args ['TRANSACTION_SOURCE']))) {
                    $language = $_SESSION ['cISOSprache'] == 'ger' ? 'DE' : 'EN';
                    if ($language == 'DE') {
                        include_once('../includes/plugins/heidelpay_standard/version/106/paymentmethod/template/heidelpay_ddMail_de.tpl');
                    } else {
                        include_once('../includes/plugins/heidelpay_standard/version/106/paymentmethod/template/heidelpay_ddMail_en.tpl');
                    }
                    $repl = array(
