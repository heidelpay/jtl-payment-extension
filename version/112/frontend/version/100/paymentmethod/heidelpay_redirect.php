...
if (! empty ( $_SESSION ['hpLastPost'] )) {
	...
	if ($_SESSION ['hpModuleMode'] == 'AFTER')
		$next = $successURL;
	...
	if (! empty ( $_GET ['cancel'] ))
		$next = 'heidelpay_fail.php?cancel=1';
	if (! empty ( $_GET ['hperror'] ))
		$next = 'heidelpay_fail.php?hperror=' . $_GET ['hperror'];
		// echo '<pre>'.print_r($_SESSION, 1).'</pre>';
...
<html>
<head>
<title>Heidelpay Redirect</title>
</head>
<body onLoad="document.forms[0].submit()">
	<center>
		...
		<br>
		<br>
		<h2>Ihre Daten werden &uuml;bertragen...</h2>
		<br> <img src="<?=$base?>bilder/ladebalken.gif">
		<form action="<?=$base.$next?>" method="post" style="display: none"
			target="_top">
<?foreach($_POST AS $k => $v){?>
  <?if (is_array($v)){?>
    <input type="text" name="<?=$k?>[<?=key($v)?>]"
				value="<?=current($v)?>">
  <?} else {?>
