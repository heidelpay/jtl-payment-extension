...
include_once('../includes/plugins/heidelpay_standard/version/105/paymentmethod/class.heidelpay_sbase.php');
...
class heidelpay_standard extends heidelpay_sbase
{
    ...
    function preparePaymentProcess($order)
    {
      ...
      if (empty($myZV)) $myZV = $bestellung->Zahlungsart->cModulId;
      ...
      if (empty($amount)) $amount = $_SESSION["Warenkorb"]->gibGesamtsummeWaren(1);
      ...
      if (class_exists('FB')) FB::log($data);
      // Response auf Notify.php umlenken.
      if (strpos($_SERVER['REMOTE_ADDR'], '127.0.0') === false) $data['FRONTEND.RESPONSE_URL'] = $notifyURL;
    ...
    function handleNotification($order, $paymentHash, $args)
    {
      ...
      } else if ($args['FRONTEND_REQUEST_CANCELLED'] == 'true'){
  ...
  function finalizeOrder($order, $hash, $args)
    ...
    function equalAddress($order)/*{{{*/
    {
      if ($order->Lieferadresse == 0) return true; // Liefer und Rechnungsadresse gleich
      $diffs = 0;
    ...
    function prepaymentText($res,$lang = 'EN') {
    	if ($lang == 'DE') {
