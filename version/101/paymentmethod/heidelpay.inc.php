...
if (!isset($_REQUEST['3ds'])){
  ...
  if (empty($_SESSION['Kunde']) && $_POST['kKunde'] > 0) $_SESSION['Kunde'] = new Kunde($_POST['kKunde']);
  ...
  if ($debug) echo '<pre>resp('.print_r($hp->response, 1).')</pre>';
  ...
  if ((($payCode == 'cc' && HPCC_MODULE_MODE == 'DIRECT') 
    ...
    && ($payMethod == 'DB' || $payMethod == 'PA')){
      // Bei DB f�r CC / DC / DD keinen IFrame anzeigen
      if ($isBestellAgain) $smarty->assign('heidelpay_iframe', $src);
      else $hp->setStatus('SUCCESS', $orderId);
  } else {
    ...
    if ($isBestellAgain) $smarty->assign('heidelpay_iframe', $src);
    else $smarty->assign('heidelpay_iframe', $hpIframe); 
  }
}