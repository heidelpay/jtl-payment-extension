...
class heidelpay_sbase extends ServerPaymentMethod
{
  ...
  var $pluginName = "heidelpay_standard";
  var $url;
  ...
  function heidelpay_sbase()/*{{{*/
  {
	$this->pageURL = gibShopURL();
...
//	if ($this->isHTTPS()) $this->pageURL = 'https://'.$_SERVER['HTTP_HOST'].'/';
  ...
  function prepareData($orderId, $amount, $currency, $payCode, $userData, $lang, $mode = 'DB', $capture = false, $uniqueId = NULL)/*{{{*/
  {
    ...
    if (class_exists('FB')) FB::log($bestellung);
...
#    $this->pageURL = constant('HP'.$this->actualPaymethod.'_PLUGIN_URL');
	$this->pageURL = gibShopURL() . "/" . PFAD_PLUGIN . $this->pluginName . "/" . PFAD_PLUGIN_VERSION . $this->getPluginVersion() . "/" . PFAD_PLUGIN_FRONTEND;
    ...
    if ($this->actualPaymethod == 'PP'){
		...
		if (class_exists('FB')) FB::log($bsParams);
  ...
  function getBillsafeBasket($order)/*{{{*/
  {
    $items = $order->Positionen;
    if (class_exists('FB')) FB::log($items);
    $i = 0;
    if ($items) {
      foreach($items as $id => $item) {
        ...
        } else if ($item->nPosTyp == C_WARENKORBPOS_TYP_VERSANDPOS){ // Shipping
          $parameters[$prefix.'.POSITION'] 				= $i;
   ...
   */
  function setStatus($type, $orderId, $amount = NULL)/*{{{*/
  {
    $status = '';
    switch($type){
    case 'SUCCESS':
    ...
    if ($status != ''){
      $sql = 'UPDATE `tbestellung` 
        SET `dBezahltDatum` = NOW(),
          `cStatus` = "'.mysql_real_escape_string($status).'",
          `cAbgeholt` = "N"
